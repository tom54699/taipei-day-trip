...
class Booking(db.Model):
    ...
    def get_order_data_by_id(orderNumber):
        trip_lists = []
        query_booking_lists = (
            db.session.query(Orders, Booking, Attraction)
            .select_from(Orders)
            .join(Booking)
            .join(Attraction)
            .filter(Orders.order_number == orderNumber)
            .all()
        )
    ...
    def get_payed_bookingId(booking_id, member_email):
        query_order_number = Booking.query.filter(
            Booking.id == booking_id,
            Booking.member_email == member_email,
            Booking.order_number != None,
        ).all()
